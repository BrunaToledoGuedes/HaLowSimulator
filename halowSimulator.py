...
class Node:
	...
	def run(self):
		...
		while True:
			...
			tempo = time.time()
			if (self.id == 10):
				packets.write(str(self.id) + '\t' + str(1) + '\t' + str(intervalToNextPacket) + '\t' + str(tempo) + '\n') #linha adicionada para atender ao algoritmo HMR dia 26/06/2021
			self.log(">>>>", str(args.length))
			...
			while True:
				...
				if self.env.now - lastDifsAttempt < DIFS:
					...
					self.log("<<<<<< tempo >>>>>>", str(args.length))
					self.state = self.STATE_IDLE
					needsBackoff = True
...
parser.add_argument("-r", "--rate", help="average packet generation rate for each node in packet/us", type=float, default=0.0000012019231)
parser.add_argument("-l", "--length", help="simulation length in us", type=float, default=24e6) #3e6 #2e7us = 20 segundos 24e6=24seg)
...
try:
	...
	# Isso pode acontecer em ambientes muito específicos ou se o processo terminar rapidamente.
	# Para fins de simulação normal, é improvável, mas é bom ter uma captura.
	print("Erro: Não foi possível obter o processo atual para monitoramento de memória.")
	current_process = None # Define como None para evitar NameError posteriormente

# Medir a memória ANTES de iniciar a simulação principal
mem_before_sim_mb = 0
if current_process: # Só tente acessar se current_process foi inicializado com sucesso
	mem_before_sim_mb = current_process.memory_info().rss / (1024 * 1024)
#parser.add_argument("-ler", "--lerCoordenada", help="leitura das coordenadas anteriores", default=None)
intervals = open('intervals.txt', 'w') #linha adicionada para atender ao ns-3 06/03/2025
packets = open('packets.txt', 'w') #linha adicionada para atender ao ns-3 06/03/2025
...
if args.groupsFromFile != None:
	...
	GroupFileTXT = open('GroupFileHMR.TXT', 'w') 
